...
USER kong
CMD /bin/sh -c 'envsubst "\${SUPABASE_ANON_KEY} \${SUPABASE_SERVICE_KEY} \${DASHBOARD_USERNAME} \${DASHBOARD_PASSWORD}" < /home/kong/kong.yml.template > /home/kong/kong.yml && kong prepare -p /usr/local/kong && /usr/local/openresty/nginx/sbin/nginx -p /usr/local/kong -c nginx.conf'
