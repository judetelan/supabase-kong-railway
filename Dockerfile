...
USER kong
CMD /bin/sh -c 'echo "PORT is: ${PORT}" && export KONG_PROXY_LISTEN="0.0.0.0:${PORT:-8000}" && echo "KONG_PROXY_LISTEN is: $KONG_PROXY_LISTEN" && envsubst "\${SUPABASE_ANON_KEY} \${SUPABASE_SERVICE_KEY} \${DASHBOARD_USERNAME} \${DASHBOARD_PASSWORD}" < /home/kong/kong.yml.template > /home/kong/kong.yml && kong prepare -p /usr/local/kong && echo "=== nginx.conf listen directive ===" && grep -A2 "listen" /usr/local/kong/nginx.conf | head -20 && echo "=== Starting nginx ===" && /usr/local/openresty/nginx/sbin/nginx -p /usr/local/kong -c nginx.conf'
